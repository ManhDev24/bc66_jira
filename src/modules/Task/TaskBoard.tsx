...
import { Avatar, Breadcrumb, Col, List, Modal, Row, Select, Tooltip, Typography } from 'antd'
...
import { getProjectDetailApi, setProjectDetailNullAction } from '../../redux/slices/project_slices'
import { createTaskApi, getAllTaskTypesApi, setTaskErrorAction, updateTaskStatusApi } from '../../redux/slices/task_slices'
...
const ProjectManagement: React.FC = () => {
  ...
  const { projectDetail, projectError, projectMembers } = useSelector((state) => state.projectReducer)
  ...
  const handleDragEnd = (result) => {
    ...
    const draggedItem = {
      ...clonedProject.lstTask[source.droppableId - 1].lstTaskDeTail[source.index],
    }
  ...
  return (
    <div className="p-3 container ">
      ...
      <div className="pt-10">
        ...
        <div className="flex">
          ...
          <div className="w-3/4">
              ...
              {projectDetail?.members.map((member) => {
                return (
                  <Tooltip key={member.userId} title={member.name} placement="top">
                    <Avatar src={member.avatar} alt={member.name} className="mr-1" />
                  </Tooltip>
                )
              })}
            </span>
            <button
              ...
              onClick={openModal}
            >
              <span role="img" aria-label="plus" className="anticon anticon-plus translate-plus">
        ...
        <Row gutter={16}>
          <DragDropContext onDragEnd={handleDragEnd}>
            {clonedProjectDetail?.lstTask?.map((listTaskItem) => {
              return (
                <Col xs={{ span: 24 }} sm={{ span: 12 }} lg={{ span: 6 }} key={listTaskItem.statusId} className="mb-4">
                  <div
                    style={{
                      ...
                      {(provided) => {
                        return (
                          <div ref={provided.innerRef} {...provided.droppableProps}>
                            {listTaskItem.lstTaskDeTail.map((listTaskDetailItem, index) => {
                              return <TaskItem key={listTaskDetailItem.taskId} listTaskDetailItem={listTaskDetailItem} index={index} onClick={handleClickTaskItem(listTaskDetailItem)} />
                            })}
                            ...
                            {listTaskItem.statusName === 'BACKLOG' && (
                              <div>
                                {!showNewTaskTextarea && (
                                  <button onClick={() => setShowNewTaskTextarea(true)} className="btn btn-primary d-flex align-items-center mt-2">
                                ...
                                {showNewTaskTextarea && (
                                  <>
                                    <div className={`bg-white border-2 mt-1 rounded${formik.errors.taskName ? ' border-red-500 focus:border-red-500' : ' border-blue-400 focus:border-blue-400'}`}>
                                      <textarea
                                        rows="2"
                                        maxLength="255"
                                          ...
                                          return (
                                            ...
                                            >
                                              <div className="flex justify-start items-center">
                                                {type.id === 1 && <BugOutlined style={{ color: 'red' }} className="me-2" />}
                                                {type.id === 2 && <CheckOutlined style={{ color: '#4b92ff' }} className="me-2" />}
                                    ...
                                    {formik.errors.taskName && <div className="text-red-500">{formik.errors.taskName}</div>}
                                  </>
                                )}
                              </div>
                            )}
        ...
        {selectedTask && <EditTaskModal visible={showEditTaskModal} onCancel={handleCancelEditTask} task={selectedTask} />}

        {projectDetail && <AddMemberModal visible={showAddMembersModal} onCancel={handleCancelUpdateMembers} project={projectDetail} onFetchProject={handleFetchProject} showFooter={false} />}
      </div>
    </div>
  )
}
